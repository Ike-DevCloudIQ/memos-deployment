...
# IAM Role for pod service accounts (IRSA)
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(aws_eks_cluster.main.identity[0].oidc[0].issuer, "https://", "")}:sub" = "system:serviceaccount:default:memos"
          }
        }
      }
    ]
...
# Allow pods to access RDS
resource "aws_iam_role_policy" "pod_rds_access" {
  name = "${var.project_name}-pod-rds-access"
...
# Allow pods to access ECR
resource "aws_iam_role_policy" "pod_ecr_access" {
  name = "${var.project_name}-pod-ecr-access"
  role = aws_iam_role.pod_execution_role.id
